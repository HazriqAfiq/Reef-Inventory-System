            ...
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" 
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-rose-700 mt-0.5 font-medium">{{ session('error') }}</p>
                    </div>
                </div>
            ...
            <a href="{{ Auth::check() && Auth::user()->isAdmin() ? route('admin.dashboard') : (Auth::check() ? route('reseller.dashboard') : '/') }}"
               class="flex items-center gap-3 transition-transform hover:scale-[1.02]">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] leading-none mt-1.5 truncate">{{ $part2 }}</p>
                </div>
            </a>
        ...
        <div class="border-t border-gray-100 p-5 lg:p-6 bg-white/50 backdrop-blur-sm shrink-0 relative z-10 transition-colors hover:bg-white/80">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-[13px] font-black text-gray-900 truncate tracking-tight">{{ Auth::user()->name }}</p>
        ...
        <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-8 shrink-0 relative">
            <div class="flex items-center gap-3">
                ...
                @isset($title)
                    ...
                    <h1 class="text-xs sm:text-sm font-bold tracking-wider uppercase text-gray-900 truncate">{{ $title }}</h1>
                @endisset
            </div>
            ...
            <a href="{{ Auth::check() && Auth::user()->isAdmin() ? route('admin.dashboard') : (Auth::check() ? route('reseller.dashboard') : '/') }}"
               class="lg:hidden absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                ...
                @endif
                <span class="text-[13px] font-black text-gray-900 tracking-tight uppercase truncate max-w-[120px]">{{ $brandName }}</span>
            </a>
            ...
            <div class="flex items-center gap-2 md:gap-4">
                ...
                <div class="hidden sm:flex items-center gap-2 text-gray-500">
                    ...
                    <span class="text-[12px] font-black text-gray-900 tracking-widest uppercase">{{ now()->format('d M Y') }}</span>
                </div>
            </div>
        </header>
