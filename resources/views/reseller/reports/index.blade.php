<x-app-layout title="My Performance Reports">
    <div class="relative animate-fade-in">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-3 shrink-0">
                ...
                <a href="{{ route('reseller.reports.pdf') }}" 
                   data-no-spa
                   download
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                ...
                <div>
                    <h3 class="text-2.5xl font-black text-gray-900 tracking-tight tabular-nums truncate">RM{{ number_format($totalProcurementSpend, 2) }}</h3>
                    <div class="flex items-baseline justify-between gap-2 mt-2 leading-none">
                        <p class="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Total spent across {{ $totalSourcedOrders }} restocks</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                ...
                <div>
                    <h3 class="text-2.5xl font-black text-gray-900 tracking-tight tabular-nums truncate">RM{{ number_format($averageOrderValue, 2) }}</h3>
                ...
                <div>
                    ...
                    <div class="mt-3.5 shrink-0">
                        <div class="w-full h-1 bg-gray-100 rounded-full overflow-hidden">
                            <div class="h-full bg-emerald-500 rounded-full transition-all duration-500" style="width: {{ $goalProgress }}%"></div>
                        </div>
                        <span class="text-[8px] text-gray-400 font-bold uppercase tracking-widest mt-1 block leading-none">
                            Spent RM{{ number_format($currentMonthSpend, 2) }} of RM{{ number_format($monthlyGoal ?: 1000, 2) }} Goal
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                ...
                <div>
                    <h3 class="text-2.5xl font-black text-gray-900 tracking-tight tabular-nums truncate">{{ number_format($totalStockUnits) }} Bottles</h3>
                ...
                <div>
                    <div class="space-y-1">
                        <div class="flex items-baseline justify-between">
                            ...
                            <span class="text-sm font-black text-gray-900 tabular-nums">RM{{ number_format($stockValuationCost, 2) }}</span>
                        </div>
                        <div class="flex items-baseline justify-between">
                            <span class="text-[10px] font-bold text-gray-400 uppercase">Retail (RSP):</span>
                            <span class="text-sm font-black text-indigo-600 tabular-nums">RM{{ number_format($stockValuationRetail, 2) }}</span>
                        </div>
                    </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                ...
                <div>
                    <h3 class="text-2.5xl font-black text-rose-600 tracking-tight tabular-nums truncate">
                        +RM{{ number_format($potentialProfitMargin, 2) }}
                    </h3>
                    <div class="mt-2 flex items-center gap-2">
                        <span class="px-1.5 py-0.5 rounded bg-rose-50 border border-rose-100 text-[8px] font-black text-rose-600 uppercase tracking-wider leading-none">
                            {{ number_format($averageMarginPercentage, 1) }}% Average Margin
                        </span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8 items-stretch">
            ...
            <div class="lg:col-span-4 bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                ...
                <div class="h-[150px] md:h-[180px] lg:h-[200px] relative flex items-center justify-center">
                    @if(collect($orderStats)->sum('count') === 0)
                        <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
                            ...
                            @if($item->product)
                                ...
                                <tr class="hover:bg-gray-50/60 transition-colors">
                                    ...
                                    <td class="py-4 px-6 flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <a href="{{ route('reseller.products.show', $item->product->slug) }}" class="font-bold text-gray-900 hover:text-indigo-600 hover:underline transition-colors block truncate">{{ $item->product->name }}</a>
                                            <div class="flex items-center gap-1.5 text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-0.5">
                                                ...
                                                <span>&middot;</span>
                                                <span>{{ optional($item->product->category)->name ?? 'No Category' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <!-- Units -->
                                    ...
                                    <td class="py-4 px-4 text-right text-gray-500 tabular-nums font-semibold">
                                        RM{{ number_format($item->product->wholesale_price, 2) }}
                                    </td>
                                    <!-- RSP -->
                                    <td class="py-4 px-4 text-right text-indigo-600 tabular-nums font-semibold">
                                        RM{{ number_format($item->product->retail_price, 2) }}
                                    </td>
                                    <!-- Profit Margin -->
                                    <td class="py-4 px-4 text-right tabular-nums font-bold text-emerald-600">
                                        ...
                                        <span class="block text-[8px] font-black text-emerald-500 uppercase tracking-wider leading-none mt-1">
                                            {{ number_format($unitProfitPercent, 1) }}% Margin
                                        </span>
                                    </td>
                                    <!-- Sourced investment -->
                                    <td class="py-4 px-6 text-right font-black text-gray-900 tabular-nums">
                                        RM{{ number_format($item->total_spend, 2) }}
                                    </td>
                                </tr>
                            @endif
                        @empty
    ...
    <script>
        ...
        if (lineCtx) {
            const trendLabels = @json($monthlyTrend->pluck('month_name'));
            const trendData = @json($monthlyTrend->pluck('total_spend'));
        ...
        if (doughnutCtx) {
            const rawStats = @json($orderStats);
            const statusLabels = [];
            const statusCounts = [];
