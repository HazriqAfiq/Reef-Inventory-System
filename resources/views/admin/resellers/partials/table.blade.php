                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-sm font-bold text-gray-900 leading-none">{{ $reseller->name }}</p>
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-1 truncate max-w-[150px]">{{ $reseller->email }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-center font-bold text-gray-900 tabular-nums">
                        {{ number_format($reseller->orders_count) }}
                    </td>

                    {{-- Revenue --}}
                    <td class="px-6 py-4 text-right whitespace-nowrap font-black text-gray-900 tabular-nums">
                        RM{{ number_format($reseller->orders_sum_total_price ?? 0, 2) }}
                    </td>

                    {{-- Joined --}}
                    <td class="px-6 py-4 hidden sm:table-cell text-right whitespace-nowrap">
                        <p class="text-xs font-bold text-gray-900 leading-none">{{ $reseller->created_at->format('d M Y') }}</p>
                        <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1">{{ $reseller->created_at->diffForHumans() }}</p>
                    </td>

                    {{-- Actions --}}
                    <td class="px-6 py-4 text-right whitespace-nowrap">
                        <div class="flex items-center justify-end gap-2.5">
                            <a href="{{ route('admin.resellers.edit', $reseller) }}" class="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-50 border border-gray-100 text-gray-400 hover:bg-black hover:text-white hover:border-black transition-all hover:scale-105 active:scale-95" title="Edit">
...
@if($resellers->nextPageUrl())
    <div id="admin-resellers-load-more-wrapper" class="px-6 py-5 border-t border-gray-50 flex justify-center">
        <button id="btn-admin-resellers-load-more" 
                data-next-url="{{ $resellers->nextPageUrl() }}" 
