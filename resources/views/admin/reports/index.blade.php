<x-app-layout title="Reports & Analytics">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                        <div>
                            ...
                            <h3 class="text-2xl font-black text-gray-900 tracking-tight tabular-nums truncate">RM{{ number_format($totalPaidRevenue, 2) }}</h3>
                        </div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                        <div>
                            ...
                            <h3 class="text-2xl font-black text-gray-900 tracking-tight tabular-nums truncate">RM{{ number_format($averageOrderValue, 2) }}</h3>
                        </div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group duration-300 flex flex-col justify-between h-full">
                        <div>
                            ...
                            <h3 class="text-2xl font-black text-gray-900 tracking-tight tabular-nums truncate">RM{{ number_format($currentMonthRevenue, 2) }}</h3>
                        </div>
                        <p class="text-[9px] text-rose-600 font-bold uppercase tracking-wider mt-2 leading-none">Current cycle ({{ $currentMonthOrders }} orders)</p>
                    </div>
                </div>
                            ...
                            <tbody class="divide-y divide-gray-100 text-xs font-medium text-gray-700">
                                @foreach($orderStats as $status => $data)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-center font-bold text-gray-900 tabular-nums">{{ $data['count'] }}</td>
                                        <td class="px-6 py-4 text-right font-black text-gray-900 tabular-nums">RM{{ number_format($data['total'], 2) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col mb-10">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-100 text-xs font-medium text-gray-700">
                                @forelse($bestSellingProducts as $index => $prod)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-3">
                                                <span class="text-[10px] font-black text-gray-400">#{{ $index + 1 }}</span>
                                                <div>
                                                    <div class="font-bold text-gray-900">{{ $prod->name }}</div>
                                                    <div class="text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-0.5 font-mono">{{ $prod->sku }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            @elseif($prod->stock < 50)
                                                <span class="inline-flex px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider border bg-amber-50 text-amber-600 border-amber-100 font-mono">{{ $prod->stock }} left</span>
                                            @else
                                                <span class="inline-flex px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider border bg-emerald-50 text-emerald-600 border-emerald-100 font-mono">{{ $prod->stock }} units</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-center font-bold text-gray-900 tabular-nums">{{ $prod->total_qty_sold ?? 0 }}</td>
                                        <td class="px-6 py-4 text-right font-black text-gray-900 tabular-nums">RM{{ number_format($prod->total_revenue ?? 0, 2) }}</td>
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <div class="w-16 bg-gray-100 rounded h-1.5 overflow-hidden print:hidden border border-gray-150">
                                                    <div class="bg-black h-full" style="width: {{ $prod->revenue_share }}%"></div>
                                                </div>
                                                <span class="font-bold text-gray-900 text-[10px] font-mono">{{ number_format($prod->revenue_share, 1) }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                ...
                                @forelse($topResellers as $reseller)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-6 py-4">
                                            ...
                                            <div class="text-[9px] text-gray-400 font-bold uppercase tracking-wider mt-0.5 font-mono truncate max-w-[200px]">{{ $reseller->email }}</div>
                                        </td>
                                        <td class="px-6 py-4 text-center font-bold text-gray-900 tabular-nums">{{ $reseller->orders_count }}</td>
                                        <td class="px-6 py-4 text-right">
                                            @if($reseller->monthly_goal)
                                                <span class="inline-flex px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider border bg-gray-50 text-gray-600 border-gray-100 font-mono">{{ $reseller->monthly_goal }} Units</span>
                                            @else
                                                <span class="text-gray-400 text-[10px]">None</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-right font-black text-gray-900 tabular-nums">RM{{ number_format($reseller->total_spend ?? 0, 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
                            ...
                            <tbody class="divide-y divide-rose-100 text-xs font-medium text-gray-700">
                                @forelse($dormantResellers as $reseller)
                                    <tr class="hover:bg-rose-50/20 transition-colors">
                                        <td class="px-6 py-4 font-bold text-gray-900">{{ $reseller->name }}</td>
                                        <td class="px-6 py-4 text-center">
                                            <span class="inline-flex px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider border bg-rose-50 text-rose-600 border-rose-100">
                                                @if($reseller->last_order_date)
                                                    {{ now()->diffInDays(\Carbon\Carbon::parse($reseller->last_order_date)) }} days inactive
                                                @else
                                                    All-time
                                                @endif
                                            </span>
                                        ...
                                        <td class="px-6 py-4 text-right font-bold text-gray-500 font-mono">
                                            {{ $reseller->last_order_date ? \Carbon\Carbon::parse($reseller->last_order_date)->format('d M Y') : 'Never' }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
            ...
            <div x-show="activeTab === 'inventory'" x-cloak class="space-y-8 print:block">
                ...
                <div class="bg-white rounded-2xl border border-rose-100 shadow-sm overflow-hidden flex flex-col mb-10">
                    <div class="px-6 py-4 border-b border-rose-100 bg-rose-50/10 shrink-0 flex items-center justify-between">
                        ...
                        <span class="text-[9px] font-black text-rose-600 bg-rose-50 border border-rose-100/50 px-2.5 py-1 rounded-lg uppercase tracking-widest font-mono">{{ $outOfStock->count() }} SKUs</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                ...
                <div class="bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden flex flex-col mb-10">
                    <div class="px-6 py-4 border-b border-amber-100 bg-amber-50/10 shrink-0 flex items-center justify-between">
                        ...
                        <span class="text-[9px] font-black text-amber-600 bg-amber-50 border border-amber-100/50 px-2.5 py-1 rounded-lg uppercase tracking-widest font-mono">{{ $lowStock->count() }} SKUs</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                            ...
                            <tbody class="divide-y divide-amber-100 text-xs font-medium text-gray-700">
                                @forelse($lowStock as $item)
                                    <tr class="hover:bg-amber-50/20 transition-colors">
                                        <td class="px-6 py-4 text-gray-900">
                                            <span class="font-mono text-amber-600 font-black mr-2">{{ $item->sku }}</span> <span class="font-bold">{{ $item->name }}</span>
                                        </td>
                                        <td class="px-6 py-4 text-center font-bold text-gray-900 font-mono">{{ $item->stock }} units</td>
                                        <td class="px-6 py-4 text-right">
