        ...
        <div class="px-8 py-6 border-b border-gray-50 bg-gray-50/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @elseif($product->stock < 50)
                    <span class="inline-flex px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest bg-amber-50 text-amber-600 border border-amber-100">Low Stock · {{ $product->stock }}</span>
                @else
                    <span class="inline-flex px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest bg-emerald-50 text-emerald-600 border border-emerald-100">In Stock · {{ $product->stock }}</span>
                @endif
            </div>
        </div>

        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-12">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 mb-8">
                    @foreach($product->images as $image)
                        <div class="relative group aspect-square rounded-2xl overflow-hidden border border-gray-100 transition-all"
                             ...
                             :class="primaryType === 'existing' && primaryId === {{ $image->id }} ? 'ring-2 ring-black border-transparent' : 'hover:border-gray-300'">
                            ...
                            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 px-4">
                                <button type="button" @click="setPrimary('existing', {{ $image->id }})" class="w-full py-2 bg-white text-black text-[8px] font-bold uppercase tracking-widest rounded-lg hover:bg-gray-100 transition-all">Set Primary</button>
                                <button type="button" @click="deletedImages.push({{ $image->id }})" class="w-full py-2 bg-rose-600 text-white text-[8px] font-bold uppercase tracking-widest rounded-lg hover:bg-rose-700 transition-all">Remove</button>
                            </div>
                            <div x-show="primaryType === 'existing' && primaryId === {{ $image->id }}" class="absolute top-2 left-2 bg-black text-white text-[8px] font-bold uppercase tracking-widest px-2 py-1 rounded-lg">Primary</div>
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="space-y-2">
                        ...
                        <input id="retail_price" name="retail_price" type="number" step="0.01" value="{{ old('retail_price', $product->retail_price) }}"
                               class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all">
                    ...
                    <div class="space-y-2">
                        ...
                        <input id="wholesale_price" name="wholesale_price" type="number" step="0.01" value="{{ old('wholesale_price', $product->wholesale_price) }}"
                               class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all">
                    ...
                    <div class="space-y-2">
                        ...
                        <input id="stock" name="stock" type="number" value="{{ old('stock', $product->stock) }}" min="0"
                               class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all">
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div class="space-y-2">
                        ...
                        <input id="top_note" name="top_note" type="text" value="{{ old('top_note', $product->top_note) }}" placeholder="Citrus, Bergamot..." class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input id="heart_note" name="heart_note" type="text" value="{{ old('heart_note', $product->heart_note) }}" placeholder="Damask Rose, Jasmine..." class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all">
                    </div>
                    <div class="space-y-2">
                        ...
                        <input id="base_note" name="base_note" type="text" value="{{ old('base_note', $product->base_note) }}" placeholder="Sandalwood, Musk..." class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all">
                    </div>
                    <div class="space-y-2">
                        ...
                        <select id="fragrance_family" name="fragrance_family" class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all cursor-pointer">
                            ...
                            <option value="fresh" {{ old('fragrance_family', $product->fragrance_family) == 'fresh' ? 'selected' : '' }}>Fresh & Aquatic</option>
                            <option value="woody" {{ old('fragrance_family', $product->fragrance_family) == 'woody' ? 'selected' : '' }}>Woody & Earthy</option>
                            <option value="floral" {{ old('fragrance_family', $product->fragrance_family) == 'floral' ? 'selected' : '' }}>Floral & Powdery</option>
                            <option value="oriental" {{ old('fragrance_family', $product->fragrance_family) == 'oriental' ? 'selected' : '' }}>Oriental & Rich</option>
                            <option value="gourmand" {{ old('fragrance_family', $product->fragrance_family) == 'gourmand' ? 'selected' : '' }}>Gourmand & Sweet</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" rows="5" class="w-full px-4 py-3 text-sm font-bold text-gray-900 bg-gray-50 border border-gray-100 rounded-xl focus:border-black focus:ring-0 transition-all resize-none">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
